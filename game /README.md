...
Quand les ennemis s'approcheront, Mario va devoir se défendre et survivre face à ces attaques de Goomba. Pour cela, il va devoir utiliser son seul pouvoir qui est les munitions en forme de carapaces. La classe projectile vas donc être utilisée. Appuyez sur la barre d'espace pour battre ces monstres.
...
Une pluie de Pic-Pic va tomber aléatoirement sur Mario. Avec les touches <- et ->, déplacez Mario pour ainsi éviter ces attaques. Dans le code, nous avons mis la fonction move_right(self) pour aller à droite et move_left(self) pour se déplacer à gauche. 
